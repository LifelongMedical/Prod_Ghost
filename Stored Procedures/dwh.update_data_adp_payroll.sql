...
-- =============================================
CREATE PROCEDURE [dwh].[update_data_adp_payroll]
AS
    BEGIN
	...
	-- interfering with SELECT statements.
        ...
        IF OBJECT_ID('dwh.data_adp_payroll') IS NOT NULL
            DROP TABLE dwh.data_adp_payroll;
...
--Important Keys 
                ...
                [Current Status] ,
                [Hire Date] ,
                CASE WHEN [Termination Date] <CAST('20120801' AS DATE) THEN NULL ELSE [Termination Date] END AS [Termination Date] ,
                [Termination Reason] ,
...
--Adding other dimensions at time of paycheck here
			   ...
			   --There are a few cases where the rate is above 200 -- this was an error in the ADP system  for one provider.  Using the logic below we null out those rates.  Otherwise it looks good.
				...
				CASE WHEN [Annual Salary] IS NULL  AND [Hourly Rate (001)] IS null THEN [Annual Salary]
				     WHEN [Rate Type Code] = 'S' 
                     THEN ROUND([Annual Salary] *(80/[Standard Hours] ), 0, 1)
                     WHEN [Rate Type Code] = 'H' THEN ROUND(   [Hourly Rate (001)] *2080, 0, 1)
                     ELSE NULL
