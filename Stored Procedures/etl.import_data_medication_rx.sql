...
-- =============================================
-- =============================================
-- Author:		<Hanife Doganay>
-- Create date: <Feb 12, 2016>
-- Updated:     <Julius Abate> 
--              <March 30, 2016>
/* 
Dependencies: 
	dwh.data_person_dp_month
	dwh.data_appointment
	dwh.data_user_v2
Description:	Imports historical pharmacy
data into DWH. Designed to be run once, with a 
separate procedure (dwh.update_data_medication_rx)
run daily to update the data. To import ALL data
historically, remove the pm.start_date filters
from the SELECT DISTINCT query in Step 1

*/
-- =============================================
CREATE PROCEDURE [etl].[import_data_medication_rx] 
...
BEGIN
	...
	--Drop the tables if they already exist, as they will be re-created
	 ...
	 IF OBJECT_ID('dwh.data_medication_rx') IS NOT NULL
...
ph.Sched_IV,
ph.Sched_V
INTO dwh.data_medication_rx --Dynamically creates the table
